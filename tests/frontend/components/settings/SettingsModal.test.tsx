...
describe("SettingsModal", () => {
  ...
  it("Cancel closes modal without calling configSave", () => {
    useUiStore.setState({ settingsOpen: true });
    render(<SettingsModal />);
  ...
  it("Save calls configSave and closes modal", async () => {
    useUiStore.setState({ settingsOpen: true });
    render(<SettingsModal />);
    fireEvent.click(screen.getByTestId("settings-save"));
    await waitFor(() => {
      expect(mockConfigSave).toHaveBeenCalledWith(expect.objectContaining(MOCK_CONFIG));
      expect(useUiStore.getState().settingsOpen).toBe(false);
    });
  });

  it("Save persists pending changes to config", async () => {
    useUiStore.setState({ settingsOpen: true });
    render(<SettingsModal />);
    // Change branch prefix on General page
