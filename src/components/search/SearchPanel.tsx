...
function SearchPanel({ projectPath, onResultClick }: SearchPanelProps) {
  ...
  return (
    <div
      ...
      className="flex h-full flex-col bg-warp-sidebar"
    >
      <div className="px-4 py-2 text-xs uppercase tracking-wider text-warp-text-dim">
        Search
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2">
        {!query && (
          <div className="px-2 py-2 text-xs text-warp-text-dim">
            Type to search
          </div>
        )}
        ...
        {results.map((result, i) => (
          ...
          >
            ...
            <div className="truncate text-xs text-warp-text-dim">
              {result.content}
            </div>
          </button>
        ))}
