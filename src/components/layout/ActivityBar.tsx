...
function ActivityBar({ changedFiles = 0 }: ActivityBarProps) {
  ...
  function iconClass(panel: "explorer" | "search" | "source-control") {
    ...
    return (
      ...
      (isActive ? "text-crucible-accent" : "text-crucible-text-dim hover:text-crucible-text")
    );
  }
  ...
  return (
    <div
      ...
      className="flex w-12 flex-shrink-0 flex-col items-center border-r border-crucible-border bg-crucible-sidebar"
    >
      ...
      >
        ...
        {changedFiles > 0 && (
          <span
            ...
            className="absolute top-1 right-1 min-w-[14px] h-[14px] rounded-full bg-crucible-accent text-black text-[9px] font-bold leading-none flex items-center justify-center px-0.5"
          >
            {badgeLabel}
          </span>
        )}
      ...
      <button
        ...
        className="mt-auto flex items-center justify-center w-12 h-12 text-crucible-text-dim hover:text-crucible-text transition-colors"
      >
        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
